...
namespace BeachCallouts
{
    ...
    [CalloutProperties("Northern Command Drug Deal", "Valandria", "0.0.2")]
    public class NC_Drugs : Callout
    {
        ...
        public NC_Drugs()
        {
            ...
            if(x <= 40)
            { 
                InitInfo(new Vector3(-176.86f, 6553.29f, 11.1f)); // 3018 Pier
            }
            else if(x > 40 && x <= 65)
            {
                InitInfo(new Vector3(1051.06f, 2650.14f, 39.55f)); // 937 behind gas station
            }
            else
            {
                InitInfo(new Vector3(1562.78f, 3589.55f, 35.36f)); // Abandoned Motel 1019
            }
            ShortName = "NC - Drug Deal in Progress";
