...
namespace BeachCallouts
{
    ...
    [CalloutProperties("Northern Command Boat Ashore", "Valandria", "0.0.2")]
    public class NC_BoatOnShore : Callout
    {
        private Vehicle car;
        ...
        public NC_BoatOnShore()
        {
            Random random = new Random();
            int x = random.Next(1, 100 + 1);
            if (x <= 40)
            {
                InitInfo(new Vector3(-2528.86f, 3907.6f, 4.43f)); // Beach 2001
            }
            else if (x > 40 && x <= 65)
            {
                InitInfo(new Vector3(874.87f, 6634.14f, 2.09f)); // Beach 3028
            }
            else
            {
                InitInfo(new Vector3(-210.63f, 6661.75f, 1.46f));
            }
            ShortName = "NC - Boat Ashore";
